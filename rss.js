 ...
 * https://github.com/assemble/assemble-contrib-rss
 *
  ...
  */
  ...
  if(!_.isUndefined(feed)) {
    ...
    var failFeed = function(prop) {
      if(feed.logging) {
        return grunt.fail.fatal(('"feed: ' + prop + '"').yellow + ' not defined in gruntfile.js for RSS feed.'.bold);
      } else {
        return;
      }
    }
     ...
     */
    var feed = new RSS({
      generator     : feed.generator      || 'Generated by Assemble, JavaScript static site generator. Visit http://assemble.io to learn more.',
      ...
      docs          : feed.docs,
      copyright     : feed.copyright      || 'Copyright' + moment().format("YYYY") + ' ' + pkg.author.name || '',
